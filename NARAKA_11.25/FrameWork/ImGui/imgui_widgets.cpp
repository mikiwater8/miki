...
void ImGui::TextV(const char* fmt, va_list args)
{
	...
	ImGuiContext& g = *GImGui;
	const char* text_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, ARRAYSIZE(g.TempBuffer), fmt, args);
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
	...
	const char* value_text_end = value_text_begin + ImFormatStringV(g.TempBuffer, ARRAYSIZE(g.TempBuffer), fmt, args);
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
	...
	const char* text_end = text_begin + ImFormatStringV(g.TempBuffer, ARRAYSIZE(g.TempBuffer), fmt, args);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
	...
	ImRect bb = bb_frame;
	bb.Expand(ImVec2(-ImClamp((FLOAT)((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f), -ImClamp((FLOAT)((bb_frame_height - 2.0f) * 0.5f), 0.0f, 3.0f)));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue)
	{
		...
		ImVec2 pad(ImMax(1.0f, (FLOAT)(square_sz / 3.6f)), ImMax(1.0f, (FLOAT)(square_sz / 3.6f)));
		window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, check_col, style.FrameRounding);
	}
	else if (*v)
	{
		const float pad = ImMax(1.0f, (FLOAT)(square_sz / 6.0f));
...
bool ImGui::CheckboxEx(const char* label, bool* v)
{
	...
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 6, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 2);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 5, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 4, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 3, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 2, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 1, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 0, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 1, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 2, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 3, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 4, check_bb.Min.y + 9), 7, ColorConvertFloat4ToU32(ImColor(155, 155, 155, 220)), 12);
	...
	if (*v)
	{
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 6, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 5, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 4, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 3, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 2, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 1, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 0, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 1, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 2, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 3, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 4, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(1.0f, 0.64705f, 0.0f, 1.0f)), 12);
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
	...
	if (!overlay)
	{
		ImFormatString(overlay_buf, ARRAYSIZE(overlay_buf), "%.0f%%", fraction * 100 + 0.01f);
		overlay = overlay_buf;
	}
...
float GetMinimumStepAtDecimalPrecision(int decimal_precision)
{
	...
	if (decimal_precision < 0)
		return FLT_MIN;
	return (decimal_precision < ARRAYSIZE(min_steps)) ? min_steps[decimal_precision] : ImPow(10.0f, (float)-decimal_precision);
}
...
bool ImGui::SliderBehavior1(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color, ImVec2 valuesize, const char* label, char* value)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
	...
	window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(grab_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), GetColorU32(ImVec4(0.78f, 0.31f, 0.05f, 1.00f)));
	if (isDown)
		window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 6, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2)), 13, GetColorU32(g.ActiveId == id ? ImVec4(0.78f, 0.31f, 0.05f, 0.4f) : ImVec4(0.78f, 0.31f, 0.05f, 0.8f)));
	window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 6, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2)), 5, GetColorU32(g.ActiveId == id ? ImVec4(0.78f, 0.31f, 0.05f, 0.4f) : ImVec4(0.78f, 0.31f, 0.05f, 0.8f)));

	window->DrawList->AddTextEx(ImVec2(grab_bb.Min.x, frame_bb.Min.y + 0), 14.f, ImColor(255, 255, 255), value, label);

	//AddText(float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL);
	...
	ImVec2 min = ImVec2(grab_bb.GetCenter().x - width / 2, frame_bb.Min.y + 2);
	ImVec2 max = ImVec2(grab_bb.GetCenter().x + width / 2, frame_bb.Min.y + 14);
	window->DrawList->AddRectFilled(min, max, GetColorU32(ImVec4(0.21f, 0.20f, 0.21f, 1.00f)), 3);
	window->DrawList->AddText(ImVec2(grab_bb.GetCenter().x - valuesize.x / 2, frame_bb.Min.y), ImColor(255, 255, 255), value, label);*/
...
int __cdecl ShrinkWidthItemComparer(const void* lhs, const void* rhs)
{
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
	...
	qsort(items, (size_t)count, sizeof(ImGuiShrinkWidthItem), ShrinkWidthItemComparer);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	char name[16];
	ImFormatString(name, ARRAYSIZE(name), "##Combo_%02d", g.BeginPopupStack.Size); // Recycle windows based on depth
...
IM_STATIC_ASSERT(ARRAYSIZE(GDataTypeInfo) == ImGuiDataType_COUNT);
...
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
	...
	if (fmt_end > fmt_start && fmt_end[-1] == 'f')
	{
		if (fmt_start == fmt && fmt_end[0] == 0)
			return "%d";
		ImGuiContext& g = *GImGui;
		ImFormatString(g.TempBuffer, ARRAYSIZE(g.TempBuffer), "%.*s%%d%s", (int)(fmt_start - fmt), fmt, fmt_end); // Honor leading and trailing decorations, but lose alignment/precision.
		return g.TempBuffer;
	}
	return fmt;
}
...
int ImGui::DataTypeFormatString(char* buf, int buf_size, ImGuiDataType data_type, const void* p_data, const char* format)
{
 ...
 void ImGui::DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size)
{
...
TYPE ImGui::RoundScalarWithFormatT(const char* format, ImGuiDataType data_type, TYPE v)
{
	...
	if (fmt_start[0] != '%' || fmt_start[1] == '%') // Don't apply if the value is not visible in the format string
		return v;
	char v_str[64];
	ImFormatString(v_str, ARRAYSIZE(v_str), fmt_start, v);
	const char* p = v_str;
	while (*p == ' ')
		p++;
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
	...
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, ARRAYSIZE(value_buf), data_type, p_data, format);
...
bool ImGui::SliderFloatEx(const char* label, float* v, float v_min, float v_max, const char* format, float power, ImVec2 size, ImVec4 color)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const bool value_changed = SliderBehavior1(frame_bb, id, v, v_min, v_max, power, decimal_precision, 0, color, CalcTextSize(value_buf, NULL, true), value_buf_end, value_buf);
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max, const char* format)
{
	if (format == NULL)
		format = "%.0f deg";
	float v_deg = (*v_rad) * 360.0f / (2 * 3.1415926f);
	bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, format, 1.0f);
	*v_rad = v_deg * (2 * 3.1415926f) / 360.0f;
	return value_changed;
}
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format)
{
	...
	char fmt_buf[32];
	char data_buf[32];
	format = ImParseFormatTrimDecorations(format, fmt_buf, ARRAYSIZE(fmt_buf));
	DataTypeFormatString(data_buf, ARRAYSIZE(data_buf), data_type, p_data, format);
	ImStrTrimBlanks(data_buf);

	ImGuiInputTextFlags flags = ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_NoMarkEdited;
	flags |= ((data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double) ? ImGuiInputTextFlags_CharsScientific : ImGuiInputTextFlags_CharsDecimal);
	bool value_changed = TempInputText(bb, id, label, data_buf, ARRAYSIZE(data_buf), flags);
	if (value_changed)
	{
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* data_ptr, const char* format)
{
	...
	if (value_changed)
		return DataTypeApplyOpFromText(data_buf, g.InputTextState.TextA.Data, data_type, data_ptr, NULL);
	return false;
}
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect& aabb, const char* label, ImGuiDataType data_type, void* data_ptr, ImGuiID id, int decimal_precision)
{
	...
	// On the first frame, g.ScalarAsInputTextId == 0, then on subsequent frames it becomes == id
	SetActiveID(g.TempInputId, window);
	SetHoveredID(0);
	FocusableItemUnregister(window);

	char buf[32];
	DataTypeFormatString(data_type, data_ptr, decimal_precision, buf, IM_ARRAYSIZE(buf));
	bool text_value_changed = InputTextEx1(label, buf, IM_ARRAYSIZE(buf), aabb.GetSize(), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_AutoSelectAll);
	if (g.TempInputId == 0)     // First frame we started displaying the InputText widget
	{
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
	...
	if (p_step != NULL)
	{
		...
		if (InputText("", buf, ARRAYSIZE(buf), flags)) // PushId(label) + "" gives us the expected ID from outside point of view
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags flags)
{
	char format[16] = "%f";
	if (decimal_precision >= 0)
		ImFormatString(format, ARRAYSIZE(format), "%%.%df", decimal_precision);
...
bool ImGui::InputData(const char* label, char* buf, ...)
{
	CHAR Buffer[260] = { 0 };
	va_list va_alist;
	va_start(va_alist, buf);
	vsprintf_s(Buffer, buf, va_alist);
	va_end(va_alist);
	return InputTextEx(label, NULL, Buffer, (int)ARRAYSIZE(Buffer), ImVec2(0, 0), 0, NULL, NULL);
}
...
namespace ImStb
{
	...
	int  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(STB_TEXTEDIT_STRING* obj, int idx) { idx++; int len = obj->CurLenW; while (idx < len && !is_word_boundary_from_left(obj, idx)) idx++; return idx > len ? len : idx; }
#else
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	if (g.ActiveId == id)
	{
		...
		const bool ignore_char_inputs = (io.KeyCtrl && !io.KeyAlt) || (io.KeySuper);
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		const bool is_redo = ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_Y)) && !is_readonly && is_undoable);

		if (IsKeyPressedMap(ImGuiKey_LeftArrow)) { state->OnKeyPressed(STB_TEXTEDIT_K_LEFT | k_mask); }
		else if (IsKeyPressedMap(ImGuiKey_RightArrow)) { state->OnKeyPressed(STB_TEXTEDIT_K_RIGHT | k_mask); }
		else if (IsKeyPressedMap(ImGuiKey_UpArrow) && is_multiline) { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - g.FontSize, 0.0f)); else state->OnKeyPressed(STB_TEXTEDIT_K_UP | k_mask); }
		else if (IsKeyPressedMap(ImGuiKey_DownArrow) && is_multiline) { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + g.FontSize, GetScrollMaxY())); else state->OnKeyPressed(STB_TEXTEDIT_K_DOWN | k_mask); }
		...
		if (render_cursor && state->CursorFollow)
		{
			...
			if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
			{
				...
				if (cursor_offset.x < state->ScrollX)
					state->ScrollX = (FLOAT)(ImMax(0.0f, cursor_offset.x - scroll_increment_x));
				else if (cursor_offset.x - inner_size.x >= state->ScrollX)
					state->ScrollX = (FLOAT)(cursor_offset.x - inner_size.x + scroll_increment_x);
			}
			else
			{
				state->ScrollX = 0.0f;
		...
		if (render_selection)
		{
			...
			for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
			{
				...
				else
				{
					...
					if (rect_size.x <= 0.0f) rect_size.x = (FLOAT)(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines
...
bool ImGui::InputTextEx1(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const bool focus_requested = FocusableItemRegister1(window, id, (flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_AllowTabInput)) == 0);    // Using completion callback disable keyboard tabbing
	...
	if (focus_requested || user_clicked || user_scrolled || user_nav_input_start)
	{
		if (g.ActiveId != id)
		{
			...
			edit_state.TextA.resize(init_buf_len + 1); // UTF-8. we use +1 to make sure that .Data isn't NULL so it doesn't crash.
			memcpy(edit_state.TextA.Data, buf, init_buf_len + 1);
			const char* buf_end = NULL;
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		else if (is_cut || is_copy)
		{
			...
			if (io.SetClipboardTextFn)
			{
				...
				edit_state.InitialTextA.resize((ie - ib) * 4 + 1);
				ImTextStrToUtf8(edit_state.InitialTextA.Data, edit_state.InitialTextA.Size, edit_state.TextW.Data + ib, edit_state.TextW.Data + ie);
				SetClipboardText(edit_state.InitialTextA.Data);
			}
			if (is_cut)
			{
				if (!edit_state.HasSelection())
	...
	if (g.ActiveId == id)
	{
		...
		{
			// Restore initial value. Only return true if restoring to the initial value changes the current buffer contents.
			if (is_editable && strcmp(buf, edit_state.TextA.Data) != 0)
			{
				apply_new_text = edit_state.TextA.Data;
				apply_new_text_length = edit_state.TextA.Size - 1;
			}
		}
		...
		if (apply_edit_back_to_user_buffer)
		{
			...
			if (is_editable)
			{
				edit_state.InitialTextA.resize(edit_state.TextW.Size * 4 + 1);
				ImTextStrToUtf8(edit_state.InitialTextA.Data, edit_state.InitialTextA.Size, edit_state.TextW.Data, NULL);
			}
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				if (event_flag)
				{
					...
					callback_data.EventKey = event_key;
					callback_data.Buf = edit_state.InitialTextA.Data;
					...
					// Read back what user may have modified
					IM_ASSERT(callback_data.Buf == edit_state.InitialTextA.Data);  // Invalid to modify those fields
			...
			// Will copy result string if modified
			if (is_editable && strcmp(edit_state.InitialTextA.Data, buf) != 0)
			{
		...
		if (apply_new_text)
		{
			...
			// If the underlying buffer resize was denied or not carried to the next frame, apply_new_text_length+1 may be >= buf_size.
			ImStrncpy(buf, edit_state.InitialTextA.Data, ImMin(apply_new_text_length + 1, buf_size));
			value_changed = true;
		}
	...
	const char* buf_display = (g.ActiveId == id && is_editable) ? edit_state.InitialTextA.Data : buf; buf = NULL;
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		const float w_item_one = ImMax(1.0f, (FLOAT)((w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) / (float)components));
		const float w_item_last = ImMax(1.0f, (FLOAT)(w_inputs - (w_item_one + style.ItemInnerSpacing.x) * (components - 1)));
	...
	else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		char buf[64];
		if (alpha)
			ImFormatString(buf, ARRAYSIZE(buf), "#%02X%02X%02X%02X", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255), ImClamp(i[3], 0, 255));
		else
			ImFormatString(buf, ARRAYSIZE(buf), "#%02X%02X%02X", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255));
		SetNextItemWidth(w_inputs);
		if (InputText("##Text", buf, ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
		{
			value_changed = true;
			char* p = buf;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	float bars_triangles_half_sz = (FLOAT)(bars_width * 0.20f);
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		if (IsItemActive())
		{
			...
			if (initial_dist2 >= (wheel_r_inner - 1) * (wheel_r_inner - 1) && initial_dist2 <= (wheel_r_outer + 1) * (wheel_r_outer + 1))
			{
				...
				H = ImAtan2(current_off.y, current_off.x) / 3.1415926f * 0.5f;
				if (H < 0.0f)
					H += 1.0f;
				value_changed = value_changed_h = true;
			}
			float cos_hue_angle = ImCos(-H * 2.0f * 3.1415926f);
			float sin_hue_angle = ImSin(-H * 2.0f * 3.1415926f);
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		for (int n = 0; n < 6; n++)
		{
			const float a0 = (n) / 6.0f * 2.0f * 3.1415926f - aeps;
			const float a1 = (n + 1.0f) / 6.0f * 2.0f * 3.1415926f + aeps;
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
	...
	if (BeginPopup("Copy"))
	{
		int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
		char buf[64];
		ImFormatString(buf, ARRAYSIZE(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)", col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
		if (Selectable(buf))
			SetClipboardText(buf);
		ImFormatString(buf, ARRAYSIZE(buf), "(%d,%d,%d,%d)", cr, cg, cb, ca);
		if (Selectable(buf))
			SetClipboardText(buf);
		ImFormatString(buf, ARRAYSIZE(buf), "#%02X%02X%02X", cr, cg, cb);
		if (Selectable(buf))
			SetClipboardText(buf);
		if (!(flags & ImGuiColorEditFlags_NoAlpha))
		{
			ImFormatString(buf, ARRAYSIZE(buf), "#%02X%02X%02X%02X", cr, cg, cb, ca);
			if (Selectable(buf))
				SetClipboardText(buf);
		}
		EndPopup();
...
bool ImGui::TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args)
{
	...
	ImGuiContext& g = *GImGui;
	const char* label_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, ARRAYSIZE(g.TempBuffer), fmt, args);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	if (display_frame)
	{
		...
		frame_bb.Min.x -= (FLOAT)(window->WindowPadding.x * 0.5f - 1.0f);
		frame_bb.Max.x += (FLOAT)(window->WindowPadding.x * 0.5f);
	}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	const float spacing_L = (FLOAT)(spacing_x * 0.50f);
	const float spacing_U = (FLOAT)(spacing_y * 0.50f);
...
void ImGui::Value(const char* prefix, float v, const char* float_format)
{
	...
	{
		char fmt[64];
		ImFormatString(fmt, ARRAYSIZE(fmt), "%%s: %s", float_format);
		Text(fmt, prefix, v);
	}
	else
	{
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
	{
		...
		popup_pos = ImVec2(pos.x - 1.0f - (FLOAT)(style.ItemSpacing.x * 0.5f), pos.y - style.FramePadding.y + window->MenuBarHeight());
		window->DC.CursorPos.x += (FLOAT)(style.ItemSpacing.x * 0.5f);
		...
		PopStyleVar();
		window->DC.CursorPos.x += (FLOAT)(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
	}
	else
	{
		// Menu inside a menu
		...
		float min_w = window->DC.MenuColumns.DeclColumns(label_size.x, 0.0f, (FLOAT)(g.FontSize * 1.20f)); // Feedback to next frame
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
	...
	else
	{
		...
		float min_w = window->DC.MenuColumns.DeclColumns(label_size.x, shortcut_w, (FLOAT)(g.FontSize * 1.20f)); // Feedback for next frame
		float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
...
int __cdecl TabItemComparerByVisibleOffset(const void* lhs, const void* rhs)
{
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
	...
	if ((flags & ImGuiTabBarFlags_Reorderable) && !(tab_bar->Flags & ImGuiTabBarFlags_Reorderable) && tab_bar->Tabs.Size > 1 && tab_bar->PrevFrameVisible != -1)
		qsort(tab_bar->Tabs.Data, tab_bar->Tabs.Size, sizeof(ImGuiTabItem), TabItemComparerByVisibleOffset);
	...
	{
		const float separator_min_x = tab_bar->BarRect.Min.x - (FLOAT)(window->WindowPadding.x * 0.5f);
		const float separator_max_x = tab_bar->BarRect.Max.x + (FLOAT)(window->WindowPadding.x * 0.5f);
...
void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
	...
	if (width_excess > 0.0f && (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown))
	{
		...
		for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
			tab_bar->Tabs[g.ShrinkWidthBuffer[tab_n].Index].Width = (FLOAT)(g.ShrinkWidthBuffer[tab_n].Width);
	}
	else
	{
...
float ImGui::TabBarCalcMaxTabWidth()
{
	ImGuiContext& g = *GImGui;
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
	...
	// Layout
	size.x = tab->Width;
	window->DC.CursorPos = tab_bar->BarRect.Min + ImVec2((FLOAT)(tab->Offset - tab_bar->ScrollingAnim), 0.0f);
	ImVec2 pos = window->DC.CursorPos;
	ImRect bb(pos, pos + size);
	...
	if (hovered && g.HoveredIdNotActiveTimer > 0.50f && bb.GetWidth() < tab->ContentWidth)
	{
		...
		bb.Max.x = bb.Min.x + (FLOAT)(ImLerp(bb.GetWidth(), tab->ContentWidth, ImSaturate((g.HoveredIdNotActiveTimer - 0.40f) * 6.0f)));
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
	...
	if (flags & ImGuiTabItemFlags_UnsavedDocument)
	{
		...
		ImVec2 unsaved_marker_pos(ImMin(bb.Min.x + frame_padding.x + label_size.x + 2, text_pixel_clip_bb.Max.x), bb.Min.y + frame_padding.y + (FLOAT)(-g.FontSize * 0.25f));
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
	...
	window->DC.CursorPos.x = (FLOAT)(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x);
