...
int __cdecl rect_height_compare(const void *a, const void *b)
{
...
int __cdecl rect_original_order(const void *a, const void *b)
{
	const stbrp_rect *p = (const stbrp_rect *)a;
	const stbrp_rect *q = (const stbrp_rect *)b;
...
static int stbrp_pack_rects(stbrp_context *context, stbrp_rect *rects, int num_rects)
{
	...
	for (i = 0; i < num_rects; ++i) {
		rects[i].was_packed = !(rects[i].x == 65535 && rects[i].y == 65535);
