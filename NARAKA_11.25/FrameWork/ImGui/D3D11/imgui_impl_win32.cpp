...
DXGI_SWAP_CHAIN_DESC g_d3dDes = { NULL };
...
bool ImGui_ImplWin32_Init()
{
	...
	{
		if (g_pd3dSwapChain && g_pd3dSwapChain->GetDesc(&g_d3dDes) == S_OK)
		{
			...
			io.ImeWindowHandle = g_d3dDes.OutputWindow;
			...
			if (g_OriginalWndProc == NULL)
				g_OriginalWndProc = (WNDPROC)SetWindowLongPtrA(g_d3dDes.OutputWindow, GWLP_WNDPROC, (LONG_PTR)ImGui_ImplWin32_WndProcHandler);
...
void ImGui_ImplWin32_Shutdown()
{
	...
	RtlSecureZeroMemory(&g_d3dDes, sizeof(g_d3dDes));
...
void ImGui_ImplWin32_NewFrame()
{
	...
	if (GetForegroundWindow() == g_d3dDes.OutputWindow && GetCursorPos(&CursorPos) && ScreenToClient(g_d3dDes.OutputWindow, &CursorPos))
	{
