...
struct ImBitVector
{
	...
	bool TestBit(int n) const { return ImBitArrayTestBit(Storage.Data, n); }
	void SetBit(int n) { ImBitArraySetBit(Storage.Data, n); }
	void ClearBit(int n) { ImBitArrayClearBit(Storage.Data, n); }
};
...
struct ImChunkStream
{
	...
	T* next_chunk(T* p) { size_t HDR_SZ = 4; p = (T*)(void*)((char*)(void*)p + chunk_size(p)); if (p == (T*)(void*)((char*)end() + HDR_SZ)) return (T*)0; return p; }
	int chunk_size(const T* p) { return ((const int*)p)[-1]; }
	T* end() { return (T*)(void*)(Buf.Data + Buf.Size); }
	int offset_from_ptr(const T* p) { const ptrdiff_t off = (const char*)p - Buf.Data; return (int)off; }
...
struct ImRect
{
	...
	void Floor() { Min.x = (FLOAT)(Min.x); Min.y = (FLOAT)(Min.y); Max.x = (FLOAT)(Max.x); Max.y = (FLOAT)(Max.y); }
...
#define IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC(_RAD,_MAXERROR) ImClamp((int)((3.1415926f * 2.0f) / ImAcos(((_RAD) - (_MAXERROR)) / (_RAD))), IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_MIN, IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_MAX)
...
struct ImDrawDataBuilder
{
	...
	void Clear() { for (int n = 0; n < ARRAYSIZE(Layers); n++) Layers[n].resize(0); }
	void ClearFreeMemory() { for (int n = 0; n < ARRAYSIZE(Layers); n++) Layers[n].clear(); }
...
namespace ImGui
{
	...
	inline ImDrawList* GetForegroundDrawList(ImGuiWindow* window) { ImGuiContext& g = *GImGui; return &g.ForegroundDrawList; } // This seemingly unnecessary wrapper simplifies compatibility between the 'master' and 'docking' branches.
	...
	void SetWindowScrollX(ImGuiWindow* window, float new_scroll_x);
	...
	void PushOverrideID(ImGuiID id); // Push given value at the top of the ID stack (whereas PushID combines old and new hashes)
	...
	bool FocusableItemRegister(ImGuiWindow* window, ImGuiID id); // Return true if focus is requested
	bool FocusableItemRegister1(ImGuiWindow* window, ImGuiID id, bool tab_stop = true);      // Return true if focus is requested
	...
	bool IsPopupOpen(ImGuiID id); // Test for id within current popup stack level (currently begin-ed into); this doesn't scan the whole popup stack!
	...
	ImVec2 FindBestWindowPosForPopupEx(const ImVec2& ref_pos, const ImVec2& size, ImGuiDir* last_dir, const ImRect& r_outer, const ImRect& r_avoid, ImGuiPopupPositionPolicy policy = ImGuiPopupPositionPolicy_Default);
	...
	// Focus scope (WIP)
	void PushFocusScope(ImGuiID id); // Note: this is storing in same stack as IDStack, so Push/Pop mismatch will be reported there.
	void PopFocusScope();
	...
	inline bool IsActiveIdUsingKey(ImGuiKey key) { ImGuiContext& g = *GImGui; return (g.ActiveIdUsingKeyInputMask & ((ImU64)1 << key)) != 0; }
	...
	void BeginColumns(const char* str_id, int count, ImGuiColumnsFlags flags = 0); // setup number of columns. use an identifier to distinguish multiple column sets. close with EndColumns().
	...
	ImGuiID GetWindowResizeID(ImGuiWindow* window, int n); // 0..3: corners, 4..7: borders
	...
	bool SliderBehavior1(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color, ImVec2 valuesize, const char* label, char* value);;
	...
	template<typename T, typename SIGNED_T, typename FLOAT_T> bool DragBehaviorT(ImGuiDataType data_type, T* v, float v_speed, T v_min, T v_max, const char* format, float power, ImGuiDragFlags flags);
	template<typename T, typename SIGNED_T, typename FLOAT_T> bool SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, T* v, T v_min, T v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb);
	template<typename T, typename FLOAT_T> float SliderCalcRatioFromValueT(ImGuiDataType data_type, T v, T v_min, T v_max, float power, float linear_zero_pos);
	...
	void DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size);
	...
	bool InputTextEx1(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
