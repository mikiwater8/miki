...
{
	Alpha = 0.85f; // Global alpha applies to everything in ImGui
	WindowPadding = ImVec2(8.f, 8.f); // Padding within a window
	WindowRounding = 0.0f; // Radius of window corners rounding. Set to 0.0f to have rectangular windows
	WindowBorderSize = 0.5f; // Thickness of border around windows. Generally set to 0.0f or 1.0f. Other values not well tested.
	WindowMinSize = ImVec2(120.f, 3.5f); // Minimum window size
	WindowTitleAlign = ImVec2(0.0f, 0.5f);// Alignment for title bar text
	WindowMenuButtonPosition = ImGuiDir_Left; // Position of the collapsing/docking button in the title bar (left/right). Defaults to ImGuiDir_Left.
	ChildRounding = 0.0f; // Radius of child window corners rounding. Set to 0.0f to have rectangular child windows
	ChildBorderSize = 0.5f; // Thickness of border around child windows. Generally set to 0.0f or 1.0f. Other values not well tested.
	PopupRounding = 0.0f; // Radius of popup window corners rounding. Set to 0.0f to have rectangular child windows
	PopupBorderSize = 0.5f; // Thickness of border around popup or tooltip windows. Generally set to 0.0f or 1.0f. Other values not well tested.
	FramePadding = ImVec2(4.f, 3.f); // Padding within a framed rectangle (used by most widgets)
	...
	ItemSpacing = ImVec2(8.f, 4.f); // Horizontal and vertical spacing between widgets/lines
	ItemInnerSpacing = ImVec2(4.f, 4.f); // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
	TouchExtraPadding = ImVec2(0.f, 0.f); // Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
	...
	ColorButtonPosition = ImGuiDir_None; // Side of the color button in the ColorEdit4 widget (left/right). Defaults to ImGuiDir_Right.
	ButtonTextAlign = ImVec2(0.5f, 0.5f);// Alignment of button text when button is larger than text.
	SelectableTextAlign = ImVec2(0.0f, 0.0f);// Alignment of selectable text when button is larger than text.
	DisplayWindowPadding = ImVec2(100.f, 100.f); // Window position are clamped to be visible within the display area by at least this amount. Only applies to regular windows.
	DisplaySafeAreaPadding = ImVec2(3.f, 3.f); // If you cannot see the edge of your screen (e.g. on a TV) increase the safe area padding. Covers popups/tooltips as well regular windows.
...
ImGuiIO::ImGuiIO()
{
	...
	for (int i = 0; i < ARRAYSIZE(MouseDownDuration); i++) MouseDownDuration[i] = MouseDownDurationPrev[i] = -1.0f;
	for (int i = 0; i < ARRAYSIZE(KeysDownDuration); i++) KeysDownDuration[i] = KeysDownDurationPrev[i] = -1.0f;
	for (int i = 0; i < ARRAYSIZE(NavInputsDownDuration); i++) NavInputsDownDuration[i] = -1.0f;
}

void ImGuiIO::AddInputCharacter(unsigned int c)
{
	InputQueueCharacters.push_back(c > 0 && c <= 0xFFFF ? (ImWchar)c : 0xFFFD);
}
...
void ImGuiIO::AddInputCharacterUTF16(ImWchar16 c)
{
	...
	if (InputQueueSurrogate != 0)
	{
		if ((c & 0xFC00) != 0xDC00) // Invalid low surrogate
			InputQueueCharacters.push_back(0xFFFD);
		else if (0xFFFF == (0xFFFF)) // Codepoint will not fit in ImWchar (extra parenthesis around 0xFFFF somehow fixes -Wunreachable-code with Clang)
			cp = 0xFFFD;
		else
...
void* ImFileLoadToMemory(const char* filename, const char* mode, size_t* out_file_size, int padding_bytes)
{
	...
	ImFileHandle f = fopen(filename, mode);
	...
	if (f != NULL)
	{
		...
		{
			file_data = ImGui::MemAlloc(file_size + padding_bytes);
...
ImGuiTextFilter::ImGuiTextFilter(const char* default_filter)
{
	if (default_filter)
	{
		ImStrncpy(InputBuf, default_filter, ARRAYSIZE(InputBuf));
		Build();
	}
	else
	{
...
bool ImGuiTextFilter::Draw(const char* label, float width)
{
	...
	bool value_changed = ImGui::InputText(label, InputBuf, ARRAYSIZE(InputBuf));
	if (value_changed)
		Build();
...
static const ImGuiStyleVarInfo GStyleVarInfo[] = {
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->Alpha)) }, // ImGuiStyleVar_Alpha
 { ImGuiDataType_Float, 2, (ImU32)((size_t)&(((ImGuiStyle*)0)->WindowPadding)) }, // ImGuiStyleVar_WindowPadding
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->WindowRounding)) }, // ImGuiStyleVar_WindowRounding
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->WindowBorderSize)) }, // ImGuiStyleVar_WindowBorderSize
 { ImGuiDataType_Float, 2, (ImU32)((size_t)&(((ImGuiStyle*)0)->WindowMinSize)) }, // ImGuiStyleVar_WindowMinSize
 { ImGuiDataType_Float, 2, (ImU32)((size_t)&(((ImGuiStyle*)0)->WindowTitleAlign)) }, // ImGuiStyleVar_WindowTitleAlign
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->ChildRounding)) }, // ImGuiStyleVar_ChildRounding
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->ChildBorderSize)) }, // ImGuiStyleVar_ChildBorderSize
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->PopupRounding)) }, // ImGuiStyleVar_PopupRounding
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->PopupBorderSize)) }, // ImGuiStyleVar_PopupBorderSize
 { ImGuiDataType_Float, 2, (ImU32)((size_t)&(((ImGuiStyle*)0)->FramePadding)) }, // ImGuiStyleVar_FramePadding
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->FrameRounding)) }, // ImGuiStyleVar_FrameRounding
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->FrameBorderSize)) }, // ImGuiStyleVar_FrameBorderSize
 { ImGuiDataType_Float, 2, (ImU32)((size_t)&(((ImGuiStyle*)0)->ItemSpacing)) }, // ImGuiStyleVar_ItemSpacing
 { ImGuiDataType_Float, 2, (ImU32)((size_t)&(((ImGuiStyle*)0)->ItemInnerSpacing)) }, // ImGuiStyleVar_ItemInnerSpacing
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->IndentSpacing)) }, // ImGuiStyleVar_IndentSpacing
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->ScrollbarSize)) }, // ImGuiStyleVar_ScrollbarSize
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->ScrollbarRounding)) }, // ImGuiStyleVar_ScrollbarRounding
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->GrabMinSize)) }, // ImGuiStyleVar_GrabMinSize
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->GrabRounding)) }, // ImGuiStyleVar_GrabRounding
 { ImGuiDataType_Float, 1, (ImU32)((size_t)&(((ImGuiStyle*)0)->TabRounding)) }, // ImGuiStyleVar_TabRounding
 { ImGuiDataType_Float, 2, (ImU32)((size_t)&(((ImGuiStyle*)0)->ButtonTextAlign)) }, // ImGuiStyleVar_ButtonTextAlign
 { ImGuiDataType_Float, 2, (ImU32)((size_t)&(((ImGuiStyle*)0)->SelectableTextAlign)) }, // ImGuiStyleVar_SelectableTextAlign
};
...
static inline bool IsWindowContentHoverable(ImGuiWindow* window, ImGuiHoveredFlags flags = NULL)
{
...
bool ImGui::FocusableItemRegister1(ImGuiWindow* window, ImGuiID id, bool tab_stop)
{
	ImGuiContext& g = *GImGui;
...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
	...
	if (g.IO.MouseClicked[1])
	{
		...
		bool hovered_window_above_modal = modal ? false : true;
	...
	for (int i = 0; i < ARRAYSIZE(g.IO.MouseDown); i++)
	{
		...
		if (g.IO.MouseClicked[i])
		{
			if ((float)(g.Time - g.IO.MouseClickedTime[i]) < g.IO.MouseDoubleClickTime)
			{
				...
				g.IO.MouseClickedTime[i] = -DBL_MAX;
			}
			else
			{
...
void ImGui::NewFrame()
{
	...
	g.IO.Framerate = (g.FramerateSecPerFrameAccum > 0.0f) ? (1.0f / (g.FramerateSecPerFrameAccum / (float)ARRAYSIZE(g.FramerateSecPerFrame))) : FLT_MAX;
	...
	Begin("##Default", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar  | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_AlwaysAutoResize);
}
...
void ImDrawDataBuilder::FlattenIntoSingleLayer()
{
	...
	for (int layer_n = 1; layer_n < ARRAYSIZE(Layers); layer_n++)
	{
...
ImVec2 ImGui::CalcTextSize(const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
	...
	// Round
	text_size.x = (FLOAT)(text_size.x + 0.95f);
	//text_size.y = (FLOAT)(text_size.x - 0.95f);
...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	char title[256];
	if (name)
		ImFormatString(title, ARRAYSIZE(title), "%s/%s_%08X", parent_window->Name, name, id);
	else
		ImFormatString(title, ARRAYSIZE(title), "%s/%08X", parent_window->Name, id);
...
static ImVec2 CalcWindowSizeAfterConstraint(ImGuiWindow* window, ImVec2 new_size)
{
	...
	if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint)
	{
		...
		new_size.x = (FLOAT)(new_size.x);
		new_size.y = (FLOAT)(new_size.y);
	}
...
static ImVec2 CalcWindowContentSize(ImGuiWindow* window)
{
	...
	ImVec2 sz;
	sz.x = (FLOAT)((window->ContentSizeExplicit.x != 0.0f) ? window->ContentSizeExplicit.x : window->DC.CursorMaxPos.x - window->DC.CursorStartPos.x);
	sz.y = (FLOAT)((window->ContentSizeExplicit.y != 0.0f) ? window->ContentSizeExplicit.y : window->DC.CursorMaxPos.y - window->DC.CursorStartPos.y);
	return sz;
}
...
static const ImGuiResizeGripDef resize_grip_def[4] = { { ImVec2(1,1), ImVec2(-1,-1), 0, 3 }, { ImVec2(0,1), ImVec2(+1,-1), 3, 6 }, { ImVec2(0,0), ImVec2(+1,+1), 6, 9 }, { ImVec2(1,0), ImVec2(-1,+1), 9,12 } };
...
static bool ImGui::UpdateWindowManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
	...
	const float grip_draw_size = (FLOAT)(ImMax(g.FontSize * 1.35f, window->WindowRounding + 1.0f + g.FontSize * 0.2f));
	const float grip_hover_inner_size = (FLOAT)(grip_draw_size * 0.75f);
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window)
{
	...
	if (border_held != -1)
	{
		...
		{
		 { ImVec2(0,+1), ImVec2(0,0), ImVec2(1,0), 3.1415926f*1.50f }, // Top
		 { ImVec2(-1,0), ImVec2(1,0), ImVec2(1,1), 3.1415926f*0.00f }, // Right
		 { ImVec2(0,-1), ImVec2(1,1), ImVec2(0,1), 3.1415926f*0.50f }, // Bottom
		 { ImVec2(+1,0), ImVec2(0,1), ImVec2(0,0), 3.1415926f*1.00f } // Left
		};
		const ImGuiResizeBorderDef& def = resize_border_def[border_held];
		ImRect border_r = GetResizeBorderRect(window, border_held, rounding, 0.0f);
		window->DrawList->PathArcTo(ImLerp(border_r.Min, border_r.Max, def.CornerPosN1) + ImVec2(0.5f, 0.5f) + def.InnerDir * rounding, rounding, def.OuterAngle - 3.1415926f * 0.25f, def.OuterAngle);
		window->DrawList->PathArcTo(ImLerp(border_r.Min, border_r.Max, def.CornerPosN2) + ImVec2(0.5f, 0.5f) + def.InnerDir * rounding, rounding, def.OuterAngle, def.OuterAngle + 3.1415926f * 0.25f);
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
	...
	if (flags & ImGuiWindowFlags_UnsavedDocument)
	{
		...
		ImVec2 off = ImVec2(0.0f, (FLOAT)(-g.FontSize * 0.25f));
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		const float resize_grip_draw_size = (FLOAT)(ImMax(g.FontSize * 1.35f, window->WindowRounding + 1.0f + g.FontSize * 0.2f));
		if (!window->Collapsed)
...
void ImGui::SetWindowSize(ImGuiWindow* window, const ImVec2& size, ImGuiCond cond)
{
	...
	if (size.x > 0.0f)
	{
		...
		window->SizeFull.x = (FLOAT)(size.x);
	}
	else
	{
		window->AutoFitFramesX = 2;
	...
	if (size.y > 0.0f)
	{
		...
		window->SizeFull.y = (FLOAT)(size.y);
	}
	else
	{
		window->AutoFitFramesY = 2;
...
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow* window, bool write)
{
	...
	{ int n = window->IDStack.Size; if (write) *p = (short)n; else p++; } // Too few or too many PopID()/TreePop()
	{ int n = window->DC.GroupStack.Size; if (write) *p = (short)n; else p++; } // Too few or too many EndGroup()

	// Global stacks
	// For color, style and font stacks there is an incentive to use Push/Begin/Pop/.../End patterns, so we relax our checks a little to allow them.
	{ int n = g.BeginPopupStack.Size; if (write) *p = (short)n; else p++; }// Too few or too many EndMenu()/EndPopup()
	{ int n = g.ColorModifiers.Size; if (write) *p = (short)n; else p++; } // Too few or too many PopStyleColor()
	{ int n = g.StyleModifiers.Size; if (write) *p = (short)n; else p++; } // Too few or too many PopStyleVar()
	{ int n = g.FontStack.Size; if (write) *p = (short)n; else p++; } // Too few or too many PopFont()
}
...
void ImGui::ItemSize(const ImVec2& size, float text_baseline_y)
{
	...
	window->DC.CursorPos.x = (FLOAT)(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x); // Next line
	window->DC.CursorPos.y = (FLOAT)(window->DC.CursorPos.y + line_height + g.Style.ItemSpacing.y); // Next line
...
void ImGui::PushMultiItemsWidths(int components, float w_full)
{
	...
	ImGuiWindow* window = g.CurrentWindow;
	const ImGuiStyle& style = g.Style;
	const float w_item_one = ImMax(1.0f, (FLOAT)((w_full - (style.ItemInnerSpacing.x) * (components - 1)) / (float)components));
	const float w_item_last = ImMax(1.0f, (FLOAT)(w_full - (w_item_one + style.ItemInnerSpacing.x) * (components - 1)));
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow* window, bool snap_on_edges)
{
	...
	scroll.x = (FLOAT)(ImMax(scroll.x, 0.0f));
	scroll.y = (FLOAT)(ImMax(scroll.y, 0.0f));
...
{
	window->ScrollTarget.x = (FLOAT)(local_x + window->Scroll.x);
...
void ImGui::SetScrollFromPosY(ImGuiWindow* window, float local_y, float center_y_ratio)
{
	...
	window->ScrollTarget.y = (FLOAT)(local_y + window->Scroll.y);
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, ImGuiTooltipFlags tooltip_flags)
{
	...
	char window_name[16];
	ImFormatString(window_name, ARRAYSIZE(window_name), "##Tooltip_%02d", g.TooltipOverrideCount);
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags flags)
{
	...
	if (flags & ImGuiWindowFlags_ChildMenu)
		ImFormatString(name, ARRAYSIZE(name), "##Menu_%02d", g.BeginPopupStack.Size); // Recycle windows based on depth
	else
		ImFormatString(name, ARRAYSIZE(name), "##Popup_%08x", id); // Not recycling, so we can close/open during the same frame
...
{
	ImGuiContext& g = *GImGui;
	g.NavAnyRequest = g.NavMoveRequest || g.NavInitRequest || g.NavWindow != NULL;
}
...
#undef NAV_MAP_KEY
	...
	for (int i = 0; i < ARRAYSIZE(g.IO.NavInputs); i++)
...
bool ImGui::SetDragDropPayload(const char* type, const void* data, size_t data_size, ImGuiCond cond)
{
	...
	{
		// Copy payload
		ImStrncpy(payload.DataType, type, ARRAYSIZE(payload.DataType));
...
bool ImGui::HotKey(const char* label, int* k)
{
	...
	if (g.HoveredId == 0 || g.HoveredId == id || g.HoveredIdAllowOverlap)
	{
		if (g.HoveredWindow == window || (false && g.HoveredRootWindow == window->RootWindow))
			if ((g.ActiveId == 0 || g.ActiveId == id || g.ActiveIdAllowOverlap) && IsMouseHoveringRect(frame_bb.Min, frame_bb.Max))
	...
	if (*k != 0 && g.ActiveId != id)
	{
		...
		strcpy(buf_display, "Please Press The Hotkey");
	}
...
void ImGui::HelpMarker(const char* desc, ...)
{
	...
	if (ImGui::IsItemHovered())
	{
		...
		va_list pArgList;
		va_start(pArgList, desc);
		_vsnprintf_s(szBuffer, sizeof(szBuffer) / sizeof(char), desc, pArgList);
		va_end(pArgList);
		ImGui::BeginTooltip();
		ImGui::PushTextWrapPos(ImGui::GetFontSize() * 35.0f);
		ImGui::TextUnformatted(szBuffer);
